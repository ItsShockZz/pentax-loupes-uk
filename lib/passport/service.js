...
export async function recordActivity(store, { passport, ip, data, entry, limits = { passport: 20, ip: 40 } }) {
  ...
  if (crm.status !== activity.crm_status) {
...
/* ---------------------------------------------------------------------- */
/* CRM hand-off. Every record is POSTed as JSON to CRM_WEBHOOK_URL with     */
/* `Authorization: Bearer <CRM_WEBHOOK_SECRET>`. Failures are recorded on   */
/* the activity (crm_status) and never fail the customer's request.        */
/* ---------------------------------------------------------------------- */

export function crmPayload(activity, passport) {
  return {
    ...
    passport:
      passport && passport.id && passport.id !== "website"
        ? { id: passport.id, reference: passport.reference, name: passport.name }
        : null,
  };
}
...
export async function forwardToCrm(activity, passport) {
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify(crmPayload(activity, passport)),
      signal: AbortSignal.timeout(8000),
    });
    if (!res.ok) {
      console.error("[passport] CRM webhook responded", res.status);
