...
import { sendMail, smtpConfig } from "./mail.js";
...
/** The shared secret: CRM_WEBHOOK_SECRET, or WEBSITE_WEBHOOK_SECRET (the CRM's name for it). */
export function crmSecret() {
  return process.env.CRM_WEBHOOK_SECRET || process.env.WEBSITE_WEBHOOK_SECRET || "";
}
...
/* ---------------------------------------------------------------------- */
/* Email alert to the UK team (lib/passport/mail.js). Never fails the       */
/* request: a mail problem is logged and the enquiry is still saved.        */
/* ---------------------------------------------------------------------- */

/** Where alerts go unless PASSPORT_NOTIFY_TO says otherwise (the UK team's inbox). */
export const DEFAULT_NOTIFY_TO = "drpuyanheydari@gmail.com";

export function notifyRecipient() {
  return process.env.PASSPORT_NOTIFY_TO || DEFAULT_NOTIFY_TO;
}
...
export async function notify({ subject, lines, replyTo }) {
  const to = notifyRecipient();
  if (!notificationsConfigured()) return false;
  try {
    const result = await sendMail({ to, subject, text: lines.join("\n"), replyTo });
    if (!result.sent && result.transport !== "none") console.error("[passport] notification failed:", result.detail || result.transport);
    return result.sent;
  } catch (err) {
